...
class DataLoader:
    ...
    def __init__(self, config):
        ...
        if config.use_bert and config.use_target_text:
            ...
            with jsonlines.open(self.BERT_TARGET_EMBEDDINGS) as reader:
                ...
                for obj in reader:
                    ...
                    bert_embedding_target = np.mean(bert_embedding_target,
                                                    axis=0)
        ...
        if config.use_target_video:
            video_features_file = h5py.File(
                'data/features/utterances_final/resnet_pool5.hdf5')
            context_video_features_file = h5py.File(
                'data/features/context_final/resnet_pool5.hdf5')
        else:
            video_features_file = None
            context_video_features_file = None

        self.parseData(dataset_json, audio_features, video_features_file,
                       context_video_features_file, text_bert_embeddings,
    ...
    def parseData(self,
        ...
        for idx, ID in enumerate(json.keys()):
            self.data_input.append(
                (json[ID]["utterance"], json[ID]["speaker"],
                 json[ID]["context"], json[ID]["context_speakers"],
                 audio_features[ID] if audio_features else None,
                 video_features_file[ID][()] if video_features_file else None,
                 context_video_features_file[ID][()]
                 if context_video_features_file else None,
                 text_bert_embeddings[idx] if text_bert_embeddings else None,
                 context_bert_embeddings[idx]
                 if context_bert_embeddings else None, json[ID]["show"]))
    ...
    def loadContextBert(
        ...
        with jsonlines.open(self.BERT_CONTEXT_EMBEDDINGS) as reader:
            ...
            for obj in reader:
                ...
                context_utterance_embeddings.append(
                    np.copy(bert_embedding_target))
    ...
    def StratifiedKFold(self, splits=5):
        ...
        if not os.path.exists(self.INDICES_FILE):
            pickle.dump(split_indices,
                        open(self.INDICES_FILE, 'wb'),
                        protocol=2)
        ...
        if os.path.exists(self.GLOVE_DICT):
            ...
            for line in open(self.config.word_embedding_path, 'r'):
                ...
                try:
                    ...
                    continue
            self.wordemb_dict[self.PAD_TOKEN] = np.zeros(
                self.config.embedding_dim)
            self.wordemb_dict[self.UNK_TOKEN] = np.random.uniform(
                -0.25, 0.25, self.config.embedding_dim)
...
class DataHelper:
    ...
    def createEmbeddingMatrix(self):
        ...
        vocab_size = len(
            self.model)  # length of filtered glove embedding words
        self.word_idx_map = word_idx_map = dict()
        self.W = W = np.zeros(shape=(vocab_size + 2, self.embed_dim),
                              dtype='float32')
    ...
    def wordToIndex(self, utterance):
        ...
        word_indices = [
            self.word_idx_map.get(word, self.UNK_ID)
            for word in nltk.word_tokenize(self.clean_str(utterance))
        ]

        #padding to max_sent_length
        word_indices = word_indices[:self.config.max_sent_length]
        word_indices = word_indices + [self.PAD_ID] * (
            self.config.max_sent_length - len(word_indices))
        assert (len(word_indices) == self.config.max_sent_length)
        return word_indices

    def getTargetBertFeatures(self, mode=None):

        utterances = self.getData(
            self.TEXT_BERT_ID, mode,
    ...
    def getAuthor(self, mode=None):
        ...
        authors = self.getData(
            self.SPEAKER_ID, mode,
        ...
        authors = [
            self.author_ind.get(author.strip(), self.UNK_AUTHOR_ID)
            for author in authors
        ]
    ...
    def vectorizeContext(self, mode=None):
        ...
        contexts = self.getData(
            self.CONTEXT_ID, mode,
        ...
        for context in contexts:
            ...
            for utterance in context[
                    -self.config.
                    max_context_length:]:  # taking latest (max_context_length) sentences
                #padding to max_sent_length
                word_indices = self.wordToIndex(utterance)
                local_context.append(word_indices)
            for _ in range(self.config.max_context_length -
                           len(local_context)):
    ...
    def pool_text(self, data):
        ...
        data_vector = [self.W[ind] for ind in data
                       if ind != 0]  # only pick up non pad words
    ...
    def oneHotOutput(self, mode=None, size=None):
        ...
        if mode == "train":
            ...
            print(
                "Set mode properly for toOneHot method() : mode = train/test")
            exit()
        return labels
        ...
        for ind, instance in enumerate(data):
            if instance.shape[1] < max_length:
                instance = np.concatenate([
                    instance,
                    np.zeros(
                        (instance.shape[0], (max_length - instance.shape[1])))
                ],
                                          axis=1)
                data[ind] = instance
    ...
    def getTargetVideoPool(self, mode=None):
        video = self.getData(
            self.TARGET_VIDEO_ID, mode,
            "Set mode properly for TargetVideo method() : mode = train/test")

        return np.array(
            [np.mean(feature_vector, axis=0) for feature_vector in video])
